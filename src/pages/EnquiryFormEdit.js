...
import { Form, Button, Row, Col, Modal, Alert, Container } from 'react-bootstrap'
import './model.css'
...
const EnquiryFormEdit = () => {
  ...
  useEffect(() => {
    if (enquiry) {
      ...
      const formattedStartDate = new Date(enquiry.StartDate).toISOString().split('T')[0]
      setFormData((prevFormData) => ({
        ...prevFormData,
        travelDestination: enquiry.destination,
        startDate: formattedStartDate,
        noOfDays: enquiry.noOfDays,
        adultPassengers: enquiry.noOfAdults,
        midPassengers: enquiry.noOfChildrenAbove6,
        childPassengers: enquiry.noOfChildrenBelow6,
        flightTrain: enquiry.travelByFlightOrTrain,
        hotelCab: enquiry.accomodationType,
  ...
  const handleSubmitEnquiry = async () => {
    try {
      ...
      let res = await MyAPI.PUT(`/admin/enquiry/${id}`, payload, token)
      let { success, message, error } = res.data || res
      if (success) {
        MyError.success(message)
        setFormData({
      ...
      {step === 1 && (
        <Row>
          ...
          <Col md={12} className="d-flex align-items-center justify-content-end">
            <Button onClick={handleSubmitEnquiry} variant="primary" className="mb-3">
              Update
            </Button>
          </Col>
