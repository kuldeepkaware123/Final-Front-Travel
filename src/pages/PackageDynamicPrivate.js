...
function PackageDynamicPrivate() {
  ...
  const fetchPrivatePackage = async (id) => {
    setLoading(true)
    try {
      let res = await MyAPI.GET(`/package/common/private/${id}`, token)
      let { success, message, error, package: packageExist } = res.data || res
      console.log(res.data)
      if (success) {
        setPackageData(packageExist)
        setStartDate(
  ...
  useEffect(() => {
    if (isPrivate && token) {
      ...
      fetchEnquiryData(token, enquiryId)
    } else {
      console.log('public')
      fetchPackage(id)
    }
  ...
  const [privatePriceText, setPrivatePriceText] = useState('')

  useEffect(() => {
    if (packageData && enquiryData) {
      if (packageData.costOptions.type !== 'total cost') {
        let val = parseInt(enquiryData.noOfAdults)
        setGroupSize(val)
        setPrivatePriceText('Per Person')
      } else {
        setPrivatePriceText('Total Cost')
      }
    }
  }, [packageData, enquiryData])
              ...
              <div className="row">
                <div className="col-12 col-lg-6">
                  <div
                    ...
                    style={{ width: '100%', minHeight: '70vh', maxHeight: '90vh' }}
                  >
                    <img
                      loading="lazy"
                          ...
                          <Row>
                            ...
                            {packageData && packageData.fixedDeparture.type === true && (
                              <Col
                                md={12}
                                style={{ background: '#244855' }}
                                className="px-3 py-2 rounded-2 mt-1 d-flex align-items-center justify-content-between"
                              >
...
export default PackageDynamicPrivate
