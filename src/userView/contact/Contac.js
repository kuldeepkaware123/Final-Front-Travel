...
function Contac() {
  ...
  const fetchEnquiryData = async () => {
    setLoading(true)
    try {
      let res = await MyAPI.GET('/user/enquiries', token)
  ...
  const filteredEnquiries = allEnquiry.filter(
    (contact) =>
      (contact.user.fistName &&
        contact.user.fistName.toLowerCase().includes(searchTerm.toLowerCase())) ||
                  ...
                  <tbody>
                    {currentEnquiries.length > 0 ? (
                      currentEnquiries.map((contact, index) => (
                        <tr key={contact.id}>
                          <td className="text-center text-dark text-truncate">
                            {index + 1 + indexOfFirstItem}
                          </td>
                          <td className="text-center text-dark text-truncate">
                            {contact.user.firstName ?? 'N/A'} {contact.user.lastName ?? ''}
                          </td>
                          <td className="text-center text-dark text-truncate">
                          ...
                          <td className="text-center text-dark text-truncate">
                            {contact.destination ?? 'N/A'}
                          </td>
                          <td className="text-center text-dark text-truncate">
                            <Button
                              variant="info"
                              onClick={() => navigate(`/user/enquiry/${contact._id}`)}
                            >
                              <FaEye size={22} />
                            </Button>
                          </td>
